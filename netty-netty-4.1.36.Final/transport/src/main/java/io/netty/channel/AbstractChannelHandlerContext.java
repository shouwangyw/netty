...
abstract class AbstractChannelHandlerContext implements ChannelHandlerContext, ResourceLeakHint {
    ...
    public EventExecutor executor() {
        // 若当前节点的executor(eventLoop)为null，则返回当前channel所绑定的eventLoop
        if (executor == null) {
    ...
    static void invokeChannelRead(final AbstractChannelHandlerContext next, Object msg) {
        ...
        EventExecutor executor = next.executor();
        if (executor.inEventLoop()) {
            next.invokeChannelRead(m);  // 调用当前节点next的channelRead()
        } else {
            executor.execute(new Runnable() {
                @Override
                public void run() {
    ...
    final boolean setAddComplete() {
        for (;;) {
            ...
            // 若当前处理器已经从pipeline移除，则直接返回false
            ...
            // 通过CAS将状态修改为 ADD_COMPLETE，即添加完成
    ...
    final void callHandlerRemoved() throws Exception {
        try {
            ...
            // 若当前节点还在pipeline上，则触发当前节点所封装处理器的handlerRemoved()方法
            if (handlerState == ADD_COMPLETE) {
                handler().handlerRemoved(this);
            }
        } finally {
            // Mark the handler as removed in any case.
            setRemoved();  // 设置当前节点的状态为 删除完毕
        }
    }
